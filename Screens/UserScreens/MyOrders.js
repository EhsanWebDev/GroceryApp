...
class MyOrders extends Component {
  ...
  renderOrders = data => {
    return data.map(item => (
      ...
      >
        <Text style={{ color: "#8e44ad", fontSize: 14 }}>
          Order Tracking ID: {item.ID}{" "}
        </Text>
          ...
          <View style={{ marginTop: 10 }}>
            {(() => {
              switch (item.ORDERSTATUS) {
                case 0:
                  return (
                    <Progress.Bar progress={0.3} width={null} height={8} />
                  );
                case 1:
                  return (
                    <Progress.Bar progress={0.7} width={null} height={8} />
                  );
                case 2:
                  return <Progress.Bar progress={1} width={null} height={8} />;
                default:
                  return null;
              }
            })()}
                ...
                case 0:
                  return (
                    <Button
                      small
                      danger
                      onPress={e => this.deleteOrder(e, item.ID)}
    ...
    return (
      <View style={{ flex: 1 }}>
        {this.props.orders.length > 0 ? (
          <ScrollView>{this.renderOrders(this.props.orders)}</ScrollView>
        ) : (
          <Text
